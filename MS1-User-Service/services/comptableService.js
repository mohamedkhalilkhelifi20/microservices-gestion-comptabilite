...
async function createComptable({
                                   ...
                                   cabinet_id, ordre_ordinal, numero_licence
                               }) {
    const db = await dbPromise;

    const cabinet = await db.get(
        `SELECT id, nom FROM cabinets WHERE id = ?`, [cabinet_id]
    );
    ...
    await db.run(
     ...
     (id, nom, email, telephone, specialite,
      cabinet_id, ordre_ordinal, numero_licence, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [id, nom, email, telephone, specialite,
            cabinet_id, ordre_ordinal, numero_licence, created_at]
    );
    ...
    return {
        id, nom, email, telephone, specialite,
        cabinet_id, cabinet_nom: cabinet.nom,
        ordre_ordinal, numero_licence, created_at
    };
}
